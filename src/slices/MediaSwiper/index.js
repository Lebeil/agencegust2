...
"use client";
import React, { useEffect, useState } from "react";
// import { createClient } from "../../prismicio"; // Supprimé - plus besoin
import EmblaCarousel from "@/components/EmblaCarousel";
import SliceHeading from "@/components/SliceHeading";
...
const MediaSwiper = ({ slice }) => {
  ...
  useEffect(() => {
    // Mock data pour éviter les erreurs Prismic
    const mockWorkItems = [
      {
        uid: "project-1",
        data: {
          ...
          cover: { url: "/assets/media/placeholder.svg" },
          preview_video: null
        }
      }
    ];
    ...
    try {
      ...
      console.warn('Error loading work items:', error);
      setWorkItems([]);
    }
  }, [slice, selectedBranch]);
